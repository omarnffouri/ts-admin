package com.transport_system.ts_admin.agora.models
...
import com.transport_system.ts_admin.agora.apis.StartCallRecordingApi
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private suspend fun loadGroupCallCallData(context: Context): Boolean {
        var innerConversation: GroupInnerConversationModel? = null
        val groupDetails = DatabaseManager.instance.fetchGroupDetails(
            conversationId = currentCall.value!!.callPayload.conversationId!!,
            context = context
        )

        //
        // checking and loading group settings
        if (groupDetails?.conversations != null) {
            innerConversation = groupDetails.conversations.firstOrNull { conversation ->
                val cid = conversation.id ?: return@firstOrNull false
                return@firstOrNull cid == currentCall.value!!.callPayload.conversationId!!
            }
            if (innerConversation != null) {
                _groupSettings.value = groupDetails.groupSettings
                AppLogger.log("Group settings found in loadGroupCallCallData")
            }
        }

        if (innerConversation == null) {
            innerConversation = DatabaseManager.instance.fetchGroupInnerConversation(
                conversationId = currentCall.value!!.callPayload.conversationId!!,
                context = context
            ) ?: return false
            AppLogger.log("loading group inner conversation but group details failed in loading data in loadGroupCallCallData")
        }
        val participants = innerConversation.participants ?: return false
    ...
    private fun loadParticipantsAndPid(participants: List<ParticipantModel>): Boolean {
        ...
        val myPid = participants.firstOrNull { participant ->
            ...
            return@firstOrNull me.userId?.toInt() == id && me.modelType == modelType
        }?.pid ?: return false
