...
class CallManager: NSObject {
    ...
    var callAcceptedWhisperSubscription : String?
    var callDeclinedWhisperSubscription : String?
    ...
    func joinOngoingCall(call : Call, completion : @escaping (Bool) -> Void){
        
        CallChannel.shared.printLog("Initializing the call channel on join ongoing call in call manager")
        ...
        callController.request(transaction) { error in
            if let error = error {
                completion(false)
                print("Error reporting ongoing call: \(error)")
            } else {
                completion(true)
                ...
                print("Ongoing call reported successfully")
        ...
        self.scheduleCallTimeout(duration: 10)
                
        AgoraManager.shared.joinOngoingCall(call: call)
    }
    ...
    func callAcceptedDeclinedElseWhere(callUuid : UUID?, accepted : Bool) {
        ...
        self.provider?.reportCall(with: uid, endedAt: Date(), reason: accepted ? .answeredElsewhere : .declinedElsewhere)
        currentCall = nil
        cancelCallTimeout()
    }
