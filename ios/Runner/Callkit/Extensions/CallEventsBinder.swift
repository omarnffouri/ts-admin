...
//  Runner
//
//  Created by Hashim Khan on 30/12/2024.
//
...
extension CallManager {
    ...
    func bindCallEvents() {
        ...
        bindCallAcceptedWhisperEvent()
        bindCallDeclinedWhisperEvent()
    }
    ...
    private func bindCallAcceptedWhisperEvent() {
        let callChannel : CallChannel = CallChannel.shared
        //
        // unbind previous event listener
        if callAcceptedWhisperSubscription != nil {
            callChannel.unbindCallAcceptedWhisperEvent(callAcceptedWhisperSubscription)
        }
        
        //
        // bind a new listener
        callAcceptedWhisperSubscription = callChannel.bindCallAcceptedWhisperEvent { [weak self] data in
            ...
            strongSelf.callAcceptedDeclinedElseWhere(callUuid: call.uuid,accepted: true)
        }
    }
    
    
    
    private func bindCallDeclinedWhisperEvent() {
        let callChannel : CallChannel = CallChannel.shared
        //
        // unbind previous event listener
        if callDeclinedWhisperSubscription != nil {
            callChannel.unbindCallDeclinedWhisperEvent(callDeclinedWhisperSubscription)
        }
        
        //
        // bind a new listener
        callDeclinedWhisperSubscription = callChannel.bindCallDeclinedWhisperEvent { [weak self] data in
            ...
            strongSelf.callAcceptedDeclinedElseWhere(callUuid: call.uuid, accepted: false)
        }
    }
