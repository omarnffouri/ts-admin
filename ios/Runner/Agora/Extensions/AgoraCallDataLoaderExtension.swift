...
extension AgoraManager {
    ...
    func loadCallData() -> Bool {
        ...
        print("My details loaded successfully on load call data function \(String(describing: myDetails.userId))")
    ...
    private func loadGroupCallData(callPayload : CallPayload,  myDetails : MyDetails) -> Bool {
        do{
            ...
            var innerConversation : GroupInnerConversationModel?
            
            if let groupDetails = try DatabaseManager.shared.fetchGroupDetails(conversationId: callPayload.conversationId!),
               let conversations = groupDetails.conversations{
                
                innerConversation = conversations.first(where: { $0.id == callPayload.conversationId! })
                
                if innerConversation != nil {
                    self.groupSettings = groupDetails.groupSettings
                }
            }
            
            if innerConversation == nil {
                guard let foundConversation = try DatabaseManager.shared.fetchGroupInnerConversation(conversationId:  callPayload.conversationId!) else{
                    print("Group conversation not found on load group call data function")
                    return false;
                }
                innerConversation = foundConversation
            }
            else{
                print("group inner conversation found in group details logic")
            }
            
            
            
            guard let conversation = innerConversation , let participants = conversation.participants else{
                print("Group participants not found on load group call data function")
                return false
            }
