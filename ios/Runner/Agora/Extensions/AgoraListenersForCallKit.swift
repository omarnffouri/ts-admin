...
extension AgoraManager {
    ...
    func callEndedFromCallManager(callUuid: UUID) {
        ...
        print("call ended called from call kit in agora manager ===> \(call.uuid)")
        
        stopCallTimer()
        if let payload = currentCall?.callPayload, (remoteUserJoined || call.wasJoiningOngoingCall) , (totalTicks > 0 || call.wasJoiningOngoingCall) {
            Task{
