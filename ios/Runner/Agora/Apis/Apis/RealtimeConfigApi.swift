...
// the local agora app id. Mirrors Dart's version-gated refresh so a cold-start
// call always joins with a fresh app id. Always returns (success, mismatch-write,
// or failure) so the call flow proceeds and degrades to the existing prefs /
// hardcoded fallback.
class RealtimeConfigApi {
    ...
    static func sync() async {
        guard let myDetails = MyDetails.loadFromSharedPrefs(),
              ...
              let url = URL(string: serverUrl + "admin/realtime-configuration") else {
            return
        }
        ...
        do {
            ...
            MyDetails.storeRealtimeConfig(agoraAppId: agoraAppId, configVersion: newVersion)
    ...
    // config_version may arrive as either a JSON string or number.
