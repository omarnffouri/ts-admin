...
import 'package:ts_admin/app/modules/chat_detail/data/models/call_notification_payload_model.dart';
import 'package:ts_admin/app/native_calling/channels/native_calling_event_channel.dart';
...
import 'package:ts_admin/app/native_calling/models/native_calling_event.dart';
...
class CallEventsController extends GetxController {
  ...
  Future<void> checkForCurrentCall() async {
    try {
      final result = await NativeCallingMethodChannel.getCurrentCall();
      if (result != null) {
        final payload = getCallPayloadFromEventData(result);
        if (payload != null && currentCall.value == null) {
  ...
  ///
  ///
  /// this function return true if current logged in user is authenticated
