...
import '../../../trucks/views/components/truck_card.dart';
...
class TrailerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String name = trailer.ownedBy?.trim() ?? '';
    final String? licencePlate = trailer.licencePlateNumber?.trim();
    final String? vin = trailer.vin?.trim();
    final int? identifier = trailer.identifier;
    final int? id = trailer.id;
    final bool hasNotes = trailer.trailerNotes?.isNotEmpty ?? false;
    ...
    return Slidable(
      ...
      closeOnScroll: true,
      groupTag: 'trailer_listing_slide_group',
              ...
              child: Column(
                ...
                children: [
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _TrailerAvatar(name: name),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Text(
                          name.isEmpty ? 'Unnamed Trailer'.tr : name,
                          maxLines: 2,
                  ...
                  Row(
                    children: [
                      if (hasNotes && onNotesTap != null)
                        _TrailerNotesButton(onTap: onNotesTap!),
                      const Spacer(),
                      Icon(
...
class _TrailerNotesButton extends StatelessWidget {
  const _TrailerNotesButton({required this.onTap});
...
class _TrailerAvatar extends StatelessWidget {
  const _TrailerAvatar({required this.name});
