...
class TrucksController extends GetxController {
  ...
  // usecase
  final getAllTrucksUsecase = sl<GetAllTrucksUsecase>();
  ...
  //Data
  final RxList<TruckEntity> trucks = RxList();
  ...
  _loadInitialData() async {
    ...
    final data = await getAllTrucks(_getRequestBody());

    if (data != null) {
      hasMoreData.value = data.hasMore ?? false;
      trucks.clear();
      trucks.addAll(data.data ?? []);
    }
    isLoading.value = false;
  }

  Future<BaseResponse<List<TruckEntity>>?> getAllTrucks(
      Map<String, dynamic> body) async {
    BaseResponse<List<TruckEntity>>? data;
    try {
      final result = await getAllTrucksUsecase.call(body);
      result.fold((BaseResponse<List<TruckEntity>> respose) {
        debugPrint('trucks length: ${respose.data?.length}');
        data = respose;
      }, (Failure r) {
  ...
  loadMoreApplications() async {
    ...
    if (data != null) {
      hasMoreData.value = data.hasMore ?? false;
      trucks.addAll(data.data ?? []);
      debugPrint('Total trucks after loading more: ${trucks.length}');
    } else {
      if (page.value > 1) {
        page.value = page.value - 1;
      }
  ...
  void refreshNoteList(String id) {
    ...
    final index = trucks.indexWhere((element) => element.id.toString() == id);

    if (index != -1) {
      final formatted =
          '${DateFormat('yyyy-MM-dd HH:mm:ss').format(now)}.${now.microsecond.toString().padLeft(6, '0')}';
      trucks[index].truckNotes?.add(
            NoteDataEntity(
  ...
  void showAddNewNoteBottomSheet({required String truckId}) {
    Get.bottomSheet(
