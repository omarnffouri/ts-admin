...
class AddDriverParticipantsController extends GetxController {
  //
  //
  //  auth controller
  ...
  final RxList<ContactEntity> drivers = RxList<ContactEntity>();
  final RxList<ContactEntity> filteredDrivers = RxList<ContactEntity>();
  ...
  Future<void> _getGroupContactsFromApi() async {
    ...
    try {
      ...
      result.fold((GroupContactsEntity contactsFromRemote) {
        _filterDrivers(contactsFromRemote);
  ...
  ///
  ///
  /// This function will filter the driver contacts from group cantacts
  /// and get only those driver contacts that are not in group participants
  void _filterDrivers(GroupContactsEntity groupContacts) {
    if ((groupContacts.applicants ?? []).isEmpty) {
      return;
    }
    //
    final group = _getGroup();
    ...
    final idsToExclude = _fetchDriverParticipantsIds(group);

    drivers.clear();
    filteredDrivers.clear();

    //
    // filtering drivers whose ids are not in 'idsToExclude' list
    drivers.addAll(groupContacts.applicants!
  ...
  List<int> _fetchDriverParticipantsIds(GroupConversationEntity group) {
    //
    ...
    for (var participantsList in listOfGroupPartcipantsList) {
      ...
      for (var participant in participantsList) {
        // checking if participant is driver and if its id is not already
