...
import 'supplier_buttom_sheet.dart';

class SupplierCard extends GetView<ShopSuppliersController> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isActive = supplier.isActive == true;
    final String name = supplier.name?.trim() ?? '';
    final bool hasRepresentative =
        (supplier.representative?.trim().isNotEmpty ?? false);
    final bool hasPhone = (supplier.phone?.trim().isNotEmpty ?? false);
    final bool hasEmail = (supplier.email?.trim().isNotEmpty ?? false);

    return Slidable(
      key: ValueKey(supplier.id),
      closeOnScroll: true,
      groupTag: 'supplier_listing_slide_group',
      endActionPane: ActionPane(
        extentRatio: 0.5,
        motion: const BehindMotion(),
        children: [
          SlidableAction(
            onPressed: (_) => controller.onDisableSupplierCicked(supplier),
      ...
      child: Semantics(
        button: true,
        label:
            name.isEmpty ? 'View supplier details' : 'View details for $name',
        child: Material(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      _SupplierStatusBadge(isActive: isActive),
                    ],
                  ),
                  if (hasRepresentative || hasPhone || hasEmail) ...[
                  ...
                  if (hasRepresentative)
                    _SupplierContactRow(
                      ...
                      text: supplier.representative!.trim(),
                      copyMessage: 'Representative copied successfully.',
                    ),
                  if (hasPhone)
                    Padding(
                      padding: EdgeInsets.only(top: hasRepresentative ? 6 : 0),
                      child: _SupplierContactRow(
                        icon: Icons.phone_outlined,
                        text: supplier.phone!.trim(),
                  ...
                  if (hasEmail)
                    Padding(
                      padding: EdgeInsets.only(
                        top: hasRepresentative || hasPhone ? 6 : 0,
                      ),
                      child: _SupplierContactRow(
                        icon: Icons.email_outlined,
                        text: supplier.email!.trim(),
...
/// Initials avatar with a polished tinted fill; falls back to a storefront
/// icon when the supplier has no usable name.
...
/// A single contact line (representative / phone / email). Tapping copies
...
// Method to show a modal bottom sheet with the full supplier details
// (address and tax reference aren't shown on the card).
void _showSupplierDetails(BuildContext context, SupplierEntity supplier) {
  Get.bottomSheet(
    Container(
      width: Get.width,
      ...
      child: SupplierButtomSheet(supplier: supplier),
    ),
  );
}
