...
class ApprovedInvoicePaymentsRequestsTab
    extends GetView<InvoicePaymentRequestsController> {
  const ApprovedInvoicePaymentsRequestsTab({super.key});

  @override
  Widget build(BuildContext context) {
    return Obx(
      () => controller.isLoadingApprovedInvoicePayments
          ? _buildLoadingView()
          : Obx(
              () => SmartRefresher(
                controller: controller.approvedRefreshController,
                header: const WaterDropMaterialHeader(),
                onRefresh: controller.handleApprovedRefresh,
                child: controller.approvedInvoicePayments.isEmpty
                    ? const NoDataView()
                    : ListView.separated(
                        itemCount: controller.approvedInvoicePayments.length,
                        itemBuilder: (BuildContext context, int index) {
                          final InvoicePaymentEntity invoice = controller
                              .approvedInvoicePayments
                              .elementAt(index);
    ...
    return Obx(
      () => AnimatedContainer(
        margin: EdgeInsets.only(
          ...
          right: 14,
          top: (index == 0) ? 14 : 0,
          bottom: (index == (controller.approvedInvoicePayments.length - 1))
              ? 14
              : 0,
        ),
        ...
        child: Column(
          children: [
            ...
            GestureDetector(
              onTap: () {
                if (controller.approvedInvoiceExpandedIndex.value == index) {
                  controller.approvedInvoiceExpandedIndex.value = -1;
                } else {
                  controller.approvedInvoiceExpandedIndex.value = index;
                }
              },
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: theme.textTheme.titleMedium?.copyWith(
                          color:
                              controller.approvedInvoiceExpandedIndex.value ==
                                          index &&
                                      (!Get.isDarkMode)
            ...
            //
            Visibility(
              visible: index == controller.approvedInvoiceExpandedIndex.value,
              child: Column(
                children: [
                  const Divider(
                    color: Colors.white,
